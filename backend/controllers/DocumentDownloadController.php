...
$renderDocumentPreview = static function (string $content, string $mimeType, string $documentName, bool $autoPrint, string $downloadHref, string $pdfStreamHref = ''): never {
    ...
    if (str_contains($normalizedMime, 'html')) {
        ...
        if ($safePdfStreamHref !== '') {
            $bodyHtml = '<iframe class="print-preview-frame" src="' . $safePdfStreamHref . '"></iframe>';
        } else {
...
if ($resolvedPath !== null) {
    ...
    if ($previewMode || $printPreview) {
        ...
        if (is_string($previewContent)) {
            ...
            $renderDocumentPreview($previewContent, $mimeType, $downloadName, $printPreview, $downloadUrl, $pdfPreviewStreamUrl);
        }
    }
...
if (is_string($blobContent) && $blobContent !== '') {
    ...
    if ($previewMode || $printPreview) {
        ...
        $renderDocumentPreview($blobContent, $mimeType, $downloadName, $printPreview, $downloadUrl, $pdfPreviewStreamUrl);
    }
    header('Content-Type: ' . $mimeType);
    header('Content-Length: ' . (string) strlen($blobContent));
    header('Content-Disposition: ' . ($serveInline ? 'inline' : 'attachment') . '; filename="' . str_replace('"', '', $downloadName) . '"');
